...
def download_docs(thread_name: str, output_dir: Path, cache_dir: Optional[str]=None) -> None:
    ...
    while sec_container.to_visit:
        if '429' in sec_container.last_url_message:
            ...
            for i in range(60):  # allow up to 60 times of failure
                try:
                    urlmsg = sec_file.download_file(form_dir, cache_dir)
                    if i > 0:
                        print(f"\nSucceeded after {i+1} trials.\n")
                    break
                except Exception as e:
                    # print(f"retry downloading {sec_file.file_download_url} due to error '{e}'\n")
                    time.sleep(random.randint(1, 10) / 10.0)
            else:
                urlmsg = "403"
                print(f"\nFailed to download {sec_file.file_download_url} after 60 trials.\n")
            if urlmsg == '200':
...
class File(FileUtils, ValidateFields):
    ...
    def download_file(self, output_dir: str, cache_dir: Optional[str]=None) -> str:
        ...
        try:
            ...
            path, msg = urllib._urlopener.retrieve(self.file_download_url, download_file_dir)
...
class FormIDX(object):
    ...
    def _get_master_zip_index(self) -> List[List[str]]:
        ...
        if self.cache_dir and os.path.exists(cache_file):
            ...
            headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.69 Safari/537.36'}
